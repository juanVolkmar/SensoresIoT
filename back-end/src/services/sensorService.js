...
export const getSensors = async () => {
  try {
    const response = await fetch('http://98.81.28.55:1026/v2/entities');
...
export const deleteSensor = async (sensorId) => {
  try {
    const response = await fetch(`http://98.81.28.55:1026/v2/entities/${sensorId}`, {
      method: 'DELETE'
    });
...
export const updateSensor = async (sensorId, partialData) => {
  try {
    const response = await fetch(`http://98.81.28.55:1026/v2/entities/${sensorId}/attrs`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json'
      },
